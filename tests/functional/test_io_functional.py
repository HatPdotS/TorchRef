...
class TestCIFReadingFunctional:
    ...
    @pytest.mark.integration
    def test_cif_atom_properties(self, shared_model):
        ...
        for col in required_cols:
            assert col in pdb.columns or col.upper() in pdb.columns, (
                f"Missing column: {col}"
            )

    @pytest.mark.integration
    def test_cif_element_types(self, shared_model):
...
class TestMTZReadingFunctional:
    ...
    @pytest.mark.integration
    def test_mtz_data_properties(self, shared_reflection_data):
        """Test that MTZ data properties are correctly loaded."""
        data = shared_reflection_data
    ...
    @pytest.mark.integration
    def test_mtz_resolution_range(self, shared_reflection_data):
    ...
    def test_cell_parameters_match(self, sample_structure_pair):
        ...
        if model_cell is not None and data_cell is not None:
            ...
            assert torch.allclose(
                model_cell.float(), data_cell.float(), rtol=0.01, atol=0.1
            )
...
class TestFrenchWilsonFunctional:
    ...
    @pytest.mark.integration
    def test_french_wilson_applied(self, shared_reflection_data):
...
class TestRfreeHandlingFunctional:
    ...
    @pytest.mark.integration
    def test_rfree_flags_loaded(self, shared_reflection_data):
        ...
        if hasattr(data, "rfree") and data.rfree is not None:
            ...
            assert data.rfree.dtype == torch.bool or torch.all(
                (data.rfree == 0) | (data.rfree == 1)
            )

    @pytest.mark.integration
    def test_rfree_fraction(self, shared_reflection_data):
...
class TestMaskHandlingFunctional:
    ...
    @pytest.mark.integration
    def test_masks_method(self, shared_reflection_data):
