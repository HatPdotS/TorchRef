...
def test_scaler_crystal_compatibility(compatibility_model_and_data) -> None:
    """Each extended crystal produces finite anisotropic scale corrections."""
    from torchref.scaling import Scaler

    model = compatibility_model_and_data["model"]
    data = compatibility_model_and_data["data"]
    ...
    assert correction.shape == (len(data.hkl),)
...
class TestScalerDeviceOperationsFunctional:
    ...
    def test_scaler_cpu_operation(self, sample_structure_pair):
        ...
        scaler = Scaler(
            model=model, data=data, nbins=10, verbose=0, device=torch.device("cpu")
        )
        scaler.setup_anisotropy_correction()
