"""Tests for the ml_sigmaa target (Read MLF with ML alpha/beta from the Scaler).

- factory dispatch / thin target
- loss math: exact reduction to the unit-variance ML target at alpha=beta=eps=1
- gradient isolation (alpha/beta are detached constants)
- end-to-end on 1DAW: alpha/beta owned by the base Scaler, maintenance resets the
...
class TestCollectionTargetsRelocated:
    ...
    def test_collection_sigmaa_base_weight(self):
        ...
        # maintenance hook present (resets the scaler's shared alpha/beta)
        assert hasattr(CollectionMLSigmaATarget, "maintenance")


@pytest.mark.unit
class TestAlphaBetaMath:
    ...
    def test_reduces_to_unit_variance_ml(self):
        """alpha=1, beta=1, eps=1 ⇒ Sigma=1 ⇒ unit-variance MLF (== ml at sigma=1)."""
        F_obs, F_calc, centric, mask = self._inputs()
        one = torch.ones_like(F_obs)
        new = ml_xray_loss_alpha_beta_math(F_obs, F_calc, one, one, centric, mask, one)
    ...
    def test_gradient_isolation(self):
        ...
        alpha = torch.full((n,), 0.7, dtype=torch.float64)  # detached constants
        beta = torch.full((n,), 30.0, dtype=torch.float64)
...
class TestSigmaAOnRealData:
    ...
    def test_alpha_beta_owned_by_base_scaler(self, refinement):
        from torchref.scaling import Scaler

        # No subclass: α/β live in the base Scaler, shared by both targets.
        assert isinstance(refinement.scaler, Scaler)
        assert hasattr(refinement.scaler, "get_alpha_beta")
        assert (
    ...
    def test_alpha_physical_and_falls_with_resolution(self, refinement):
        assert torch.isfinite(refinement.xray_target_work.forward())
        alpha, beta, eps = refinement.scaler.get_alpha_beta()
        v = refinement.reflection_data.masks().to(torch.bool)
        assert (alpha[v] > 0).all() and (alpha[v] <= 1.0 + 1e-6).all()
        assert (beta[v] > 0).all()
        abin = refinement.scaler.alpha_per_bin
        # low-resolution half more correlated than high-resolution half (robust
        # to small bin-to-bin bumps and LBFGS-scale nondeterminism).
        h = abin.numel() // 2
        assert abin[:h].mean() > abin[h:].mean()

    def test_maintenance_resets_cache(self, refinement):
        refinement.scaler.get_alpha_beta()
        assert refinement.scaler._alpha_beta_cache is not None
        refinement.xray_target_work.maintenance()
        assert refinement.scaler._alpha_beta_cache is None
