...
Certifies the eager ``vectorized_add_to_map_aniso`` reference (iso-reduction
identity + float64 gradcheck). The production variable-radius CPU/GPU splats are
covered separately by ``test_variable_radius_gpu`` and the CPU grouped-splat tests.
"""
