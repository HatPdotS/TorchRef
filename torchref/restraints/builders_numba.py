...
class PreprocessedCIF:
    ...
    def _preprocess_planes(self, planes_df: pd.DataFrame) -> Dict[str, Any]:
        ...
        for plane_id in plane_ids:
            ...
            planes_data.append({
                ...
                'sigmas': plane_atoms['sigma'].values.astype(np.float64) if 'sigma' in plane_atoms.columns else np.full(len(plane_atoms), 0.02),
            })
    ...
    def _preprocess_chirals(self, chirals_df: pd.DataFrame) -> Dict[str, np.ndarray]:
        ...
        return {
            ...
            'sigma': chirals_df['sigma'].values.astype(np.float64) if 'sigma' in chirals_df.columns else np.full(len(chirals_df), 0.2),
        }
...
class FastBondBuilder:
    ...
    def process_residue(
        ...
        count = match_bonds_numba(
            ...
            self._work_idx1, self._work_idx2,
            self._work_refs, self._work_sigmas
        )
...
class FastAngleBuilder:
    ...
    def process_residue(
        ...
        count = match_angles_numba(
            ...
            self._work_idx1, self._work_idx2, self._work_idx3,
            self._work_refs, self._work_sigmas
        )
...
class FastTorsionBuilder:
    ...
    def process_residue(
        ...
        count = match_torsions_numba(
            ...
            self._work_idx1, self._work_idx2, self._work_idx3, self._work_idx4,
            self._work_refs, self._work_sigmas, self._work_periods
        )
...
class FastChiralBuilder:
    ...
    def process_residue(
        ...
        count = match_chirals_numba(
            ...
            self._work_center, self._work_idx1, self._work_idx2, self._work_idx3,
            self._work_signs, self._work_sigmas
        )
