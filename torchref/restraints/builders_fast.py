...
class PreprocessedPDB:
    ...
    def _compute_residue_boundaries(self):
        ...
        residue_keys = np.char.add(
            np.char.add(self.chain_ids.astype('<U10'), '_'),
            self.resseqs.astype(str)
        )
    ...
    def get_altloc_conformations(self, residue_idx: int) -> Iterator[Tuple[np.ndarray, np.ndarray, str]]:
        """
...
class PreprocessedCIF:
    ...
    def _preprocess_planes(self, planes_df: pd.DataFrame) -> List[Dict]:
        ...
        for plane_id in plane_ids:
            ...
            planes_data.append({
                ...
                'sigmas': plane_atoms[sigma_col].values.astype(np.float64) if sigma_col else np.full(len(plane_atoms), 0.02),
            })
    ...
    def _preprocess_chirals(self, chirals_df: pd.DataFrame) -> Dict[str, np.ndarray]:
        ...
        return {
            ...
            'sigma': chirals_df[sigma_col].values.astype(np.float64) if sigma_col else np.full(len(chirals_df), 0.2),
        }
        ...
        for res_idx in range(pp_pdb.n_residues):
            ...
            for atom_names, atom_indices, _ in pp_pdb.get_altloc_conformations(res_idx):
                ...
                count = match_bonds_numba(
                    atom_names, atom_indices,
                    cif_bonds['atom1'], cif_bonds['atom2'],
                    cif_bonds['value'], cif_bonds['sigma'],
                    work_idx1, work_idx2, work_refs, work_sigmas
                )
                
                if count > 0:
                    all_indices.append(np.column_stack([
        ...
        for res_idx in range(pp_pdb.n_residues):
            ...
            for atom_names, atom_indices, _ in pp_pdb.get_altloc_conformations(res_idx):
                count = match_angles_numba(
                    atom_names, atom_indices,
                    cif_angles['atom1'], cif_angles['atom2'], cif_angles['atom3'],
                    cif_angles['value'], cif_angles['sigma'],
                    work_idx1, work_idx2, work_idx3, work_refs, work_sigmas
                )
        ...
        for res_idx in range(pp_pdb.n_residues):
            ...
            for atom_names, atom_indices, _ in pp_pdb.get_altloc_conformations(res_idx):
                count = match_torsions_numba(
                    atom_names, atom_indices,
                    cif_torsions['atom1'], cif_torsions['atom2'],
                    cif_torsions['atom3'], cif_torsions['atom4'],
                    cif_torsions['value'], cif_torsions['sigma'], cif_torsions['period'],
                    work_idx1, work_idx2, work_idx3, work_idx4,
                    work_refs, work_sigmas, work_periods
                )
        ...
        for res_idx in range(pp_pdb.n_residues):
            ...
            for atom_names, atom_indices, _ in pp_pdb.get_altloc_conformations(res_idx):
                count = match_chirals_numba(
                    atom_names, atom_indices,
                    cif_chirals['center'], cif_chirals['atom1'],
                    cif_chirals['atom2'], cif_chirals['atom3'],
                    cif_chirals['volume_sign'], cif_chirals['sigma'],
                    work_center, work_idx1, work_idx2, work_idx3,
                    work_signs, work_sigmas
                )
    ...
    def __init__(self, link_dict: Dict):
        ...
        if 'torsions' in link_dict and link_dict['torsions'] is not None:
            ...
            self.torsions = {
                ...
                'atom3': torsions_df['atom3'].values.astype(str),
                'atom4': torsions_df['atom4'].values.astype(str),
                'id': torsions_df['id'].values.astype(str) if 'id' in torsions_df.columns else np.full(len(torsions_df), '', dtype='<U10'),
                'value': torsions_df['value'].values.astype(np.float64),
                'sigma': torsions_df['sigma'].values.astype(np.float64),
                'period': torsions_df['period'].values.astype(np.int64) if 'period' in torsions_df.columns else np.ones(len(torsions_df), dtype=np.int64),
            }
    ...
    def build(
        ...
        for res_i_idx, res_next_idx in pairs:
            ...
            for a in range(n_angles):
                comp1, comp2, comp3 = angles['comp1'][a], angles['comp2'][a], angles['comp3'][a]
                atom1, atom2, atom3 = angles['atom1'][a], angles['atom2'][a], angles['atom3'][a]
...
class InterResidueTorsionBuilder:
    ...
    def build(
        ...
        omega_data = {'indices': [], 'references': [], 'sigmas': [], 'periods': [], 'is_proline': []}
        ...
        for res_i_idx, res_next_idx in pairs:
            ...
            for t in range(n_torsions):
                ...
                if not (atom1 in map1 and atom2 in map2 and atom3 in map3 and atom4 in map4):
                    continue
                
                idx1, idx2, idx3, idx4 = map1[atom1], map2[atom2], map3[atom3], map4[atom4]
    ...
    def build(
        ...
        for res_i_idx, res_next_idx in pairs:
            ...
            for plane_data in link_data.planes:
                ...
                for i, (comp_id, atom_name, sigma) in enumerate(zip(comp_ids, atom_names, sigmas)):
