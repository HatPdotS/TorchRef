...
Examples:
  ...
  torchref-refine-screened -s model.pdb -f reflections.mtz -o output/ \\
    ...
    if args.verbose > 0:
        ...
        print(f"  XYZ weights: {args.n_xyz_weights} samples from {args.xyz_min_weight} to {args.xyz_max_weight}")
        print(f"  ADP weights: {args.n_adp_weights} samples from {args.adp_min_weight} to {args.adp_max_weight}")
    ...
    try:
        ...
        try:
            ...
            history_data["final_statistics"]["rmsd_bonds"] = torch.sqrt((bond_devs ** 2).mean()).item()
            angle_devs, _ = refinement.restraints.angle_deviations()
            history_data["final_statistics"]["rmsd_angles"] = torch.sqrt((angle_devs ** 2).mean()).item()
        except:
            pass
