...
# This makes json.dump/dumps automatically handle StatEntry and torch tensors
  ...
  torchref-refine -s model.cif -f reflections.cif -o output/
    ...
    if args.hyperparameters.lower() != 'none':
        try:
            ...
            if args.verbose > 0:
                print(f"Applied {len(hyperparams)} hyperparameters.\n")
                sys.stdout.flush()
...
# Save refined structure factors (if available)
    ...
    if args.verbose > 0:
        ...
        # Write JSON - StatEntry and tensors are automatically serialized
