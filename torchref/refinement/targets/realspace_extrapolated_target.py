"""
Real-Space Extrapolated Target for Time-Resolved Crystallographic Refinement.

This module provides a real-space target that compares extrapolated pure-light
electron density against model density computed from the light model's Fcalc.

The extrapolation uses observed dark and light amplitudes with model-derived
phases to compute:

    F_extra = (F_obs_light * exp(i*phi_mixed) - w_dark * F_obs_dark * exp(i*phi_dark)) / w_light

This isolates the pure-light signal by removing the dark-state contribution,
scaled by the population fractions.  The resulting density is compared against
the light model's density via real-space correlation coefficient (RSCC).

Gradient Flow
-------------
- Phases from dark/mixed models: **detached** (prevents circular gradients)
- Observed amplitudes: buffers (no gradients)
- Population fractions: **gradients retained** (for fraction refinement)
- Model density from light model Fcalc: **gradients flow** through here
...
from .realspace_targets import RealSpaceTarget
