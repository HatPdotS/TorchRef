...
def read_crystallographic_info(filepath: str) -> Tuple[Optional[List[float]], Optional[str], Optional[str]]:
    """
...
def load_as_dataframe(filepath: str, skipheader: int = 0, skipfooter: int = 1) -> pd.DataFrame:
    """
    Load a PDB file into a pandas DataFrame.
    ...
    pdb = pd.read_fwf(
        filepath, names=names, colspecs=colspecs,
        skiprows=skipheader, skipfooter=skipfooter,
    ...
    anisou_colspecs = [
        (0, 6), (6, 11), (12, 16), (16, 17), (17, 20), (21, 22), (22, 26),
        (29, 35), (36, 42), (43, 49), (50, 56), (57, 63), (63, 70), (76, 78)
    ]
    anisou = pd.read_fwf(
        filepath, names=anisou_names, colspecs=anisou_colspecs,
        skiprows=skipheader, skipfooter=skipfooter,
        keep_default_na=False, na_values=['']
    )
    ...
    pdb[['x', 'y', 'z', 'occupancy', 'tempfactor']] = (
        pdb[['x', 'y', 'z', 'occupancy', 'tempfactor']].astype(float)
    )
    pdb[['altloc', 'icode']] = pdb[['altloc', 'icode']].fillna('')
    pdb['charge'] = (
        pdb['charge'].astype(str).str.strip('+')
        .str.replace('1-', '-1').str.replace('2-', '-2')
...
def write(df: pd.DataFrame, filepath: str, template: str = None) -> None:
    ...
    with open(filepath, 'w') as n:
        ...
        for i, row in df.iterrows():
            ATOM, serial, name, altloc, resname, chainid, resseq, icode, x, y, z_coord, occupancy, tempfactor, element, charge = row[[
                'ATOM', 'serial', 'name', 'altloc', 'resname', 'chainid', 'resseq',
                'icode', 'x', 'y', 'z', 'occupancy', 'tempfactor', 'element', 'charge'
            ]]
