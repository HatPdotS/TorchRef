...
def MapSymmetry(space_group, map_shape, cell_params, dtype_float=torch.float32, verbose=1, device=torch.device('cpu')):
    ...
    if compat['can_use_direct_indexing']:
        ...
        if verbose > 0:
            print(f"MapSymmetry: Using direct indexing (no interpolation) for {space_group}")
        return MapSymmetryDirect(space_group, map_shape, cell_params, dtype_float, verbose, device)
    else:
        # Use interpolation-based fallback
        if verbose > 0:
            print(f"MapSymmetry: Grid not compatible with direct indexing")
        ...
        from torchref.symmetrie.map_symmetrie_interpolation import MapSymmetry as MapSymmetryInterpolation
...
class MapSymmetryDirect(nn.Module):
    ...
    def __init__(self, space_group, map_shape, cell_params,dtype_float=torch.float32, verbose=1,device=torch.device('cpu')):
        ...
        if self.verbose > 0:
            ...
            print(f"  ✓ Using direct indexing (no interpolation)")
    ...
    def __repr__(self):
        return (f"MapSymmetryDirect(space_group='{self.space_group}', "
