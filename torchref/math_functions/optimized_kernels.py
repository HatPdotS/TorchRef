...
def fused_aniso_gaussian_density(
    ...
    # (r^T U) r
    quad_form = torch.matmul(rT_U, diff_expanded.unsqueeze(-1))  # (N_atoms, N_voxels, 4, 1, 1)
...
def warmup_cuda_operations(device='cuda'):
    ...
    dummy_map.view(-1).index_add_(0, 
                                  dummy_indices[:, 0] * 10000 + 
                                  dummy_indices[:, 1] * 100 + 
                                  dummy_indices[:, 2],
                                  dummy_values)
...
class CachedRadiusMask:
    ...
    def get_offsets(self, voxel_size: torch.Tensor, radius_angstrom: float, device: torch.device) -> torch.Tensor:
        """
        ...
        if key not in self._cache:
            ...
            # Compute distances
            distance_map = torch.sqrt(torch.sum((coords * voxel_size.unsqueeze(0)) ** 2, dim=-1))
...
def get_cached_radius_offsets(voxel_size: torch.Tensor, radius_angstrom: float, device: torch.device) -> torch.Tensor:
    """
...
def vectorized_add_to_map_optimized(
    ...
    """
    from torchref.math_functions.math_torch import smallest_diff, scatter_add_nd
    
    # Calculate squared distances with periodic boundary conditions
    diff_coords_squared = smallest_diff(
        surrounding_coords - xyz.unsqueeze(1), 
        inv_frac_matrix, 
        frac_matrix
    )
