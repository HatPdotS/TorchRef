...
class InterpolatedMLTarget(nn.Module):
    ...
    use_p1_cell : bool, optional
        If True, compute the reciprocal space grid using a cubic P1 cell
        centered on the model. This ensures rotation covariance by removing
        the lattice bias from the data's cell. Default is True (RECOMMENDED).
    ...
    When use_p1_cell=True:
    - The model is centered at origin in a cubic P1 cell
    - The reciprocal space grid is computed on this uniform Cartesian grid
    - This removes lattice bias and ensures proper rotation covariance
    """
    ...
    def __init__(
        ...
        # Pre-compute the reciprocal space grid
        if use_p1_cell:
            # Use P1 cubic cell for rotation-covariant grid
            reciprocal_grid, p1_cell_size, s_vectors = self._build_p1_reciprocal_grid(
                model, data.cell, high_resolution_limit
            )
            self.register_buffer("reciprocal_grid", reciprocal_grid)
            self.p1_cell_size = p1_cell_size
            self.register_buffer("s_vectors", s_vectors)  # Cartesian s-vectors for HKL
        else:
            # Original approach using data's cell
            model.build_complete_map()
            from torchref.math_functions.math_torch import ifft

            reciprocal_grid = ifft(model.map)
            self.register_buffer("reciprocal_grid", reciprocal_grid)
            self.p1_cell_size = None
            self.s_vectors = None
    ...
    def _build_p1_reciprocal_grid(
        ...
        """
        Build reciprocal space grid using P1 cubic cell centered on model.

        This ensures rotation covariance by computing F_calc on a uniform
        Cartesian grid that is independent of the crystal lattice.
        ...
        cell : torch.Tensor
            Original cell parameters (for computing s-vectors of HKL).
        resolution_limit : float
        ...
        grid_oversampling : int, optional
            Grid oversampling factor for finer interpolation. Default 2.

        Returns
        -------
        reciprocal_grid : torch.Tensor
            Complex reciprocal space grid on P1 cell.
        p1_cell_size : float
            Edge length of the cubic P1 cell.
        s_vectors : torch.Tensor
            Cartesian s-vectors for the observed HKL indices.
        """
        device = model.device

        # Get atomic positions and center the model
        xyz = model.xyz().to(device=device, dtype=torch.float64)
        xyz_centered = xyz - xyz.mean(dim=0)

        # Determine cubic cell size to contain model
        max_extent = xyz_centered.abs().max().item()
        p1_cell_size = 2 * (max_extent + cell_padding)

        # Determine grid size based on resolution
        # For cubic cell: d_min = a / |hkl|_max, so |hkl|_max = a / d_min
        if resolution_limit > 0:
            d_min = resolution_limit
        else:
            d_min = 2.0  # Default to 2 Å if no limit specified

        hkl_max = int(math.ceil(p1_cell_size / d_min)) + 1

        # Grid size with oversampling
        grid_size = (2 * hkl_max + 1) * grid_oversampling

        # Create the reciprocal space grid by computing F_calc at grid points
        # For cubic P1 cell: s = hkl / a
        h_range = torch.arange(-hkl_max, hkl_max + 1, device=device, dtype=torch.float32)
        k_range = torch.arange(-hkl_max, hkl_max + 1, device=device, dtype=torch.float32)
        l_range = torch.arange(-hkl_max, hkl_max + 1, device=device, dtype=torch.float32)

        # Create full HKL grid
        h_grid, k_grid, l_grid = torch.meshgrid(h_range, k_range, l_range, indexing='ij')
        hkl_grid = torch.stack([h_grid, k_grid, l_grid], dim=-1)  # (N, N, N, 3)

        # s-vectors for P1 grid: s = hkl / a
        s_grid = hkl_grid / p1_cell_size  # (N, N, N, 3) in Å⁻¹

        # Compute F_calc at all grid points
        # F(s) = Σ_j f_j exp(2πi s·r_j)
        # Use atomic numbers as approximate scattering factors
        if hasattr(model, 'pdb') and 'Z' in model.pdb.columns:
            weights = torch.tensor(
                model.pdb['Z'].values, device=device, dtype=torch.float64
            )
        else:
            weights = torch.ones(xyz_centered.shape[0], device=device, dtype=torch.float64) * 6.0

        # Flatten grid for batch computation
        s_flat = s_grid.reshape(-1, 3).to(torch.float64)  # (N³, 3)
        n_points = s_flat.shape[0]

        # Compute F_calc in batches
        batch_size = 50000
        F_calc_flat = torch.zeros(n_points, device=device, dtype=torch.complex128)

        for batch_start in range(0, n_points, batch_size):
            batch_end = min(batch_start + batch_size, n_points)
            s_batch = s_flat[batch_start:batch_end]
            ...
            F_calc_flat[batch_start:batch_end] = F_batch

        # Reshape to grid
        grid_shape = (len(h_range), len(k_range), len(l_range))
        reciprocal_grid = F_calc_flat.reshape(grid_shape)

        # Compute Cartesian s-vectors for the observed HKL indices
        # Using the original cell's B matrix
        s_vectors = self._hkl_to_cartesian_s(self.hkl, cell.to(device))

        return reciprocal_grid, p1_cell_size, s_vectors

    def _hkl_to_cartesian_s(
        self,
        hkl: torch.Tensor,
        cell: torch.Tensor,
    ) -> torch.Tensor:
        """
        Convert HKL indices to Cartesian s-vectors using cell parameters.
        ...
        -------
        torch.Tensor
            Cartesian s-vectors of shape (N, 3) in Å⁻¹.
        """
        device = hkl.device
        dtype = torch.float64

        a, b, c, alpha, beta, gamma = cell.to(dtype)
        ...
        # Build B matrix (reciprocal space metric in Cartesian frame)
    ...
    def forward(
        ...
        if self.use_p1_cell:
            # P1 cell approach: rotate Cartesian s-vectors, then convert to P1 grid indices
            # For rotation R of molecule: F'(s) = F(R^T @ s)
            # s_vectors are in Cartesian coordinates (Å⁻¹)
            s_rotated = self.s_vectors @ R  # Rotate s-vectors

            # Convert to P1 grid indices: hkl_p1 = s * p1_cell_size
            hkl_rotated = s_rotated * self.p1_cell_size

            # Interpolate from P1 grid
            F_calc_amp = self._interpolate_p1_grid(
                self.reciprocal_grid, hkl_rotated
            )
        else:
            # Original approach: rotate HKL indices directly
    ...
    def _interpolate_p1_grid(
        self,
        reciprocal_grid: torch.Tensor,
        hkl: torch.Tensor,
    ) -> torch.Tensor:
        """
        Interpolate amplitudes from P1 reciprocal space grid.

        The P1 grid is indexed by HKL where the origin is at grid center.
        ...
        hkl : torch.Tensor
            Continuous HKL positions of shape (N, 3).

        Returns
        -------
        torch.Tensor
            Interpolated amplitudes of shape (N,).
        """
        Nx, Ny, Nz = reciprocal_grid.shape
        device = reciprocal_grid.device

        # The grid is centered: index = hkl + N//2
        h = hkl[:, 0] + Nx // 2
        k = hkl[:, 1] + Ny // 2
        l = hkl[:, 2] + Nz // 2
        ...
        hd = (h - h0.float()).to(reciprocal_grid.real.dtype)
        kd = (k - k0.float()).to(reciprocal_grid.real.dtype)
        ld = (l - l0.float()).to(reciprocal_grid.real.dtype)
    ...
    def scan_rotations_batched(
        ...
        for batch_start in range(0, n_rotations, batch_size):
            ...
            if self.use_p1_cell:
                # P1 cell approach: rotate Cartesian s-vectors
                # s_vectors: (N_refl, 3), R_batch: (batch, 3, 3)
                # Result: (batch, N_refl, 3)
                s_rotated = torch.einsum("ij,bjk->bik", self.s_vectors, R_batch)

                # Convert to P1 grid indices
                hkl_rotated = s_rotated * self.p1_cell_size

                # Interpolate amplitudes using P1 grid batched method
                F_calc_batch = self._interpolate_p1_batched(
                    self.reciprocal_grid, hkl_rotated
                )
            else:
                # Original approach: rotate HKL indices directly
            ...
            for i in range(actual_batch_size):
                ...
                # Scale F_calc to match F_obs
                scale = (self.F_obs * F_calc_amp).sum() / (F_calc_amp**2).sum().clamp(
                    min=1e-10
                )
                F_calc_scaled = scale * F_calc_amp
    ...
    def _interpolate_p1_batched(
        self,
        reciprocal_grid: torch.Tensor,
        hkl_batch: torch.Tensor,
    ) -> torch.Tensor:
        """
        Batched interpolation from P1 reciprocal space grid.
        ...
        return result.reshape(batch_size, n_refl).float()
    ...
    def rotation_search(
        ...
        if verbose:
            print("Point group symmetry reduces search space to:")
            print(
...
class MLOrientationAligner:
    ...
    def align(
        ...
        if use_patterson_prescreen:
            if self.verbose > 0:
                print(
                    "Patterson pre-screening not yet implemented, using initial rotation"
                )
        ...
        final_xyz = self.target.transform_coordinates(
    ...
    def refine_orientation(
        ...
        eps_rot = (
            1e-3  # Step size for rotation gradient (larger for more stable gradient)
        )
        ...
        for iteration in range(max_iter):
            ...
            if self.verbose > 1:
                ...
                print(
                    f"  Iter {iteration + 1}: LLG = {current_llg:.2f}, grad_norm = {grad_norm:.6f}"
                )
...
class TranslationSearchTarget(nn.Module):
    ...
    def get_correlation_at_translation(
        ...
        phase_factor = torch.complex(torch.cos(trans_phase), torch.sin(trans_phase)).to(
            F_calc_sym.dtype
        )
...
class MolecularReplacementPipeline:
    ...
    def __init__(
        ...
        refinement_limit = (
            high_resolution_limit / 2 if high_resolution_limit > 0 else 0.0
        )
    ...
    def run(self) -> MRResult:
        ...
        if self.verbose > 0:
            ...
            print(
                f"  Translation (frac): [{best_trans_frac[0]:.3f}, {best_trans_frac[1]:.3f}, {best_trans_frac[2]:.3f}]"
            )
        ...
        stage_start = time.time()
        final_quat, final_trans_frac, final_llg, joint_converged = (
            self.joint_refinement(best_quat, refined_trans_frac)
        )
    ...
    def refine_rotation(
        ...
        for i, idx in enumerate(top_indices):
            ...
            if self.verbose > 1:
                print(
                    f"    Initial LLG: {llg_values[idx]:.2f} -> Refined: {refined_llg:.2f}"
                )
    ...
    def _compute_rfactor(
        ...
        if hasattr(self.data, "rfree_flags") and self.data.rfree_flags is not None:
            ...
            if len(F_obs_free) > 0:
                scale_free = (F_obs_free * F_calc_free).sum() / (
                    F_calc_free**2
                ).sum().clamp(min=1e-10)
                r_free = float(
                    (
