    ...
    def from_model(
        ...
        if mode == "auto":
            ...
            if has_atom:
                ...
                return torch.tensor(
                    (pdb["name"] == "CA").values, dtype=torch.bool
                )
            else:
...
class ClashScoreCalculator(nn.Module):
    ...
    def _get_valid_transforms(
        ...
        B = math_np.get_fractional_matrix(cell_np)
    ...
    def forward(
        ...
        # Compute fractionalization matrices
        B_inv = math_np.get_inv_fractional_matrix(cell_np)
        B_inv = torch.tensor(B_inv, dtype=dtype, device=device)
        B = math_np.get_fractional_matrix(cell_np)
        B = torch.tensor(B, dtype=dtype, device=device)

        # Convert ASU to fractional coordinates
        xyz_frac = cartesian_to_fractional_torch(xyz_selected, cell_np, B_inv)
        ...
        for transform in valid_transforms:
            ...
            xyz_mate_cart = fractional_to_cartesian_torch(xyz_mate_frac, cell_np, B)
