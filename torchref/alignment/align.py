...
from torchref.symmetry.spacegroup import get_point_group
...
class PattersonAligner:
    ...
    def compute_clash_score(
        ...
        if xyz is None:
            ...
            atom_mask = self._clash_atom_mask if model is self.model else AtomSampler.from_model(model)
        else:
    ...
    def _grid_search_rotation(
        ...
        if self.verbose > 0:
            ...
            if use_symmetry:
                print(f"    (reduced by point group symmetry)")
        ...
        if self.verbose > 1:
            print(f"    Coarse scores range: [{scores_coarse.min():.4f}, {scores_coarse.max():.4f}]")

        # Select top candidates from Stage 1
        top_coarse_indices = torch.argsort(scores_coarse, descending=True)[:n_top_coarse]
        top_coarse_quats = quaternions[top_coarse_indices]
        top_coarse_scores = scores_coarse[top_coarse_indices]

        if self.verbose > 0:
            print(f"    Top {n_top_coarse} coarse scores: {top_coarse_scores[:5].tolist()}")
        ...
        if self.verbose > 0:
            ...
            print(f"    Testing {n_local_samples} points around each of {n_top_coarse} candidates")
        ...
        if seed is not None:
            sampler_fine = VectorSampler(model_no_water, weighting=self.weighting, seed=seed + 1)
        else:
        ...
        local_quats = []
        local_source_indices = []  # Track which coarse candidate each local quat came from
        ...
        if self.verbose > 1:
            print(f"    Fine scores range: [{scores_fine.min():.4f}, {scores_fine.max():.4f}]")
    ...
    def align(
        ...
        if use_correlation_scoring and n_refine < 50:
            if self.verbose > 0:
                print(f"Note: Increasing n_refine from {n_refine} to 50 for correlation scoring")
            n_refine = 50
        ...
        if self.verbose > 0:
            ...
            print(f"Starting Patterson alignment (three-stage search)...")
            ...
            if use_correlation_scoring:
                print(f"  Correlation scoring enabled")
            if use_symmetry:
                print(f"  Using point group symmetry for grid reduction")
        ...
        if seed is not None:
            sampler_refine = VectorSampler(model_no_water, weighting=self.weighting, seed=seed + 2)
        else:
        ...
        for i, q in enumerate(top_quaternions):
            ...
            R_opt, t_opt, patterson, clash, converged = self._optimize_single_start(
                xyz_centered, idx1, idx2, R_init, t_init_frac,
                max_iter=max_iter,
        ...
        if use_correlation_scoring:
            ...
            for i, cand in enumerate(candidates):
                transform = RigidTransform.from_matrix(cand['R'].detach(), cand['t'].detach())
                try:
                    correlation = self.compute_correlation_for_transform(transform, model)
                    cand['correlation'] = correlation
                except Exception as e:
                    if self.verbose > 0:
                        print(f"  Warning: Correlation computation failed for candidate {i+1}: {e}")
            ...
            best_cand = max(candidates, key=lambda c: c['patterson'] - clash_weight * c['clash'])
        ...
        transform = RigidTransform.from_matrix(best_cand['R'].detach(), best_cand['t'].detach())
        result = AlignmentResult(
            transform=transform,
    ...
    def generate_rotation_grid(
        ...
        euler_angles = sample_angles(angular_step * (3.141592653589793 / 180.0), max_angles)
    ...
    def get_random_orientations(self, n_angles) -> torch.Tensor:
        ...
        max_angles = get_rotation_sampling_range(self.symmetry.matrices)
        euler_angles = torch.rand(n_angles, 3,device=self.device) * torch.tensor(max_angles, device=self.device).unsqueeze(0)
