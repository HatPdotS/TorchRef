#!/usr/bin/env python3
"""Benchmark GPU memory usage for Extended Figure 4.
...
Output: data/exF4_memory.csv          (peak memory per structure)
        data/exF4_memory_timeline.json (per-stage memory for selected structures)

Usage:
    # Full run (timeline + scatter):
    python benchmark_exF4_memory.py

    # Timeline only for specific structures:
    python benchmark_exF4_memory.py --timeline-only --codes 138L 3K7M 1AQ3

    # Include the OOM structures from the GPU pipeline:
    python benchmark_exF4_memory.py --include-oom
"""
...
PAPER_ROOT = BASE.parent.parent
DATA_DIR = PAPER_ROOT / "data"
STRUCTURES_JSON = PAPER_ROOT / "figure2_validation" / "structures.json"
OUT_CSV = BASE  / "exF4_memory.csv"
OUT_TIMELINE = BASE  / "exF4_memory_timeline.json"
...
def main():
    parser = argparse.ArgumentParser(description="GPU memory profiling for ExF 4")
