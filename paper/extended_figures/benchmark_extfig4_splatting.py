...
Output: data/extfig4_splatting.json

Usage:
    # CPU-only (no GPU required):
    python benchmark_extfig4_splatting.py --cpu-only

    # All (needs GPU):
    python benchmark_extfig4_splatting.py
"""
...
DATA_DIR = Path(__file__).resolve().parent.parent / "figure3_performance" / "data"
PDB_FILE = DATA_DIR / "1DAW.pdb"
MTZ_FILE = DATA_DIR / "1DAW.mtz"
OUT_JSON = Path(__file__).resolve().parent / "data" / "extfig4_splatting.json"
...
def main():
    parser = argparse.ArgumentParser(description="Splatting benchmark for ExtFig 4")
