#!/usr/bin/env python
"""Extended Figure 3: R-factor scorer consistency (REFMAC / PHENIX / TorchRef).
...
ENGINES = [
    ...
    (TORCHREF_ENGINE, "TorchRef", "#b2182b"),
]
...
def create_figure(outpath: str, dpi: int = 300):
    ...
    refiners = ["refmac", "phenix", TORCHREF_ENGINE]
    ...
    fig.suptitle("Extended Figure 3 — R-factor scorer consistency: REFMAC / "
                 "PHENIX / TorchRef (same data, same free set)", y=1.03, fontsize=18)
...
def main():
    ...
    args = p.parse_args()
    outpath = args.output or str(OUTPUT_DIR / "extended_figure3.png")
